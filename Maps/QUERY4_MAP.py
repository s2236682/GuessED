...
mapp = folium.Map(location=[55.9540697,-3.184623], max_zoom=20, zoom_start=18, tiles='cartodbdark_matter')
...
c.execute("SELECT A.SP_NAME, SDO_UTIL.TO_WKTGEOMETRY(A.ORA_GEOMETRY), B.POLY_NAME, SDO_UTIL.TO_WKTGEOMETRY(B.ORA_GEOMETRY), C.ROUTE_NAME, SDO_UTIL.TO_WKTGEOMETRY(C.ORA_GEOMETRY) FROM SEARCHPOINTS_GAME A, POLYGONS_GAME B, ROUTES_GAME C WHERE A.SP_NAME = 'D' AND B.POLY_NAME = 'Calton hill' AND C.ROUTE_NAME = 'Princes Street'")
...
<circle cx="8" cy="8" r="6" fill="orange" opacity="1" />
    ...
    folium.vector_layers.Polygon(locarray2, popup="What is this landmark?").add_to(mapp)
