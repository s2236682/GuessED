...
mapp = folium.Map(location=[55.965441,-3.209011], zoom_start=16, tiles='cartodbdark_matter')
...
c.execute("SELECT POLY_NAME, SDO_UTIL.TO_WKTGEOMETRY(ORA_GEOMETRY) FROM POLYGONS_GAME  WHERE  POLY_NAME = 'Royal Botanic Gardens'")
...
for row in c:
    ...
    folium.vector_layers.Polygon(locarray2, popup = rname).add_to(mapp)
