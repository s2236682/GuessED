...
mapp = folium.Map(location=[55.9486,-3.2005], zoom_start=17, tiles='cartodbdark_matter')
...
c.execute("SELECT A.SP_NAME, SDO_UTIL.TO_WKTGEOMETRY(A.ORA_GEOMETRY), SDO_UTIL.TO_WKTGEOMETRY(B.ORA_GEOMETRY), C.ROUTE_NAME, SDO_UTIL.TO_WKTGEOMETRY(C.ORA_GEOMETRY)  FROM SEARCHPOINTS_GAME A, POLYGONS_GAME B, ROUTES_GAME C WHERE A.SP_NAME = 'A' AND B.POLY_NAME = 'Edinburgh Castle' AND C.ROUTE_NAME = 'Royal Mile'")
