...
mapp = folium.Map(location=[55.94942,-3.19138], max_zoom=20, zoom_start=19, tiles='cartodbdark_matter')
...
c.execute("SELECT A.SP_NAME, SDO_UTIL.TO_WKTGEOMETRY(A.ORA_GEOMETRY), B.POLY_NAME, SDO_UTIL.TO_WKTGEOMETRY(B.ORA_GEOMETRY), C.ROUTE_NAME, SDO_UTIL.TO_WKTGEOMETRY(C.ORA_GEOMETRY), SDO_UTIL.TO_WKTGEOMETRY(D.ORA_GEOMETRY) FROM SEARCHPOINTS_GAME A, POLYGONS_GAME B, ROUTES_GAME C, POINTS_GAME D WHERE A.SP_NAME = 'B' AND B.POLY_NAME = 'St Giles cathedral' AND C.ROUTE_NAME = 'Royal Mile' AND D.POINT_ID < 3")
