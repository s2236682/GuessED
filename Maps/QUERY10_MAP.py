...
mapp = folium.Map(location=[55.923041, -3.175127], zoom_start=16, tiles='cartodbdark_matter')
...
c.execute("SELECT SDO_UTIL.TO_WKTGEOMETRY(A.ORA_GEOMETRY), SDO_UTIL.TO_WKTGEOMETRY(B.ORA_GEOMETRY) FROM SEARCHPOL_GAME A, POLYGONS_GAME B  WHERE A.SPOL_NAME = 'Kings Buildings' AND B.POLY_NAME ='Crew Building'")
...
for row in c:
    ...
    folium.vector_layers.Polygon(locarray2, popup="Edinburgh University King's Buildings Campus").add_to(mapp)
